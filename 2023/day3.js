...
function getResult(parts) {
    let partNbs = [];

    for(let i = 0; i < parts.length; i++) {
        let part = parts[i];
        let prevPart = i > 0 ? parts[i - 1] : null;
        let nextPart = i < parts.length - 1 ? parts[i + 1] : null;
        let partMatchs = [...part.matchAll(/\d+/g)];

        partMatchs.forEach(p => {
            let isPartNb = false;
            let l = p[0].length;
            let startIndex = p.index;
            let endIndex = startIndex + l - 1;

            if (startIndex > 0 && part.charAt(startIndex - 1) != '.' && isNaN(parseInt(part.charAt(startIndex - 1))))
                isPartNb = true;
            
            if (endIndex + 1 < part.length && part.charAt(endIndex + 1) != '.' && isNaN(parseInt(part.charAt(endIndex + 1))))
                isPartNb = true;

            if (prevPart != null) {
                let hasChar = prevPart.substring(startIndex > 0 ? startIndex - 1 : startIndex, endIndex + 2 < prevPart.length ? endIndex + 2 : endIndex + 1).replace(/(\.)|(\d)/g, '').length > 0;
                if (hasChar)
                    isPartNb = true;
            }

            if (nextPart != null) {
                let hasChar = nextPart.substring(startIndex > 0 ? startIndex - 1 : startIndex, endIndex + 2 < nextPart.length ? endIndex + 2 : endIndex + 1).replace(/(\.)|(\d)/g, '').length > 0;
                if (hasChar)
                    isPartNb = true;
            }
    ...
    return partNbs.reduce((acc, cur) => acc + cur, 0);
}

function getResult2(parts) {
    let partNbs = [];
    ...
    for(let i = 0; i < parts.length; i++) {
        ...
        let prevPart = i > 0 ? parts[i - 1] : null;
        let nextPart = i < parts.length - 1 ? parts[i + 1] : null;
        let partMatchs = [...part.matchAll(/\*/g)];
        
        partMatchs.forEach(p => {
            let startIndex = p.index > 0 ? p.index - 1 : p.index;
            let endIndex = p.index < part.length - 1 ? p.index + 1 : p.index;
            let nbs = [];

            if (startIndex != p.index && !isNaN(parseInt(part.charAt(startIndex)))) {
                nbs.push(+part.substring(0, p.index + 1).match(/\d+\*/g).pop().slice(0, -1));
            }
            
            if (endIndex != p.index && !isNaN(parseInt(part.charAt(endIndex)))) {
                nbs.push(+part.substring(p.index, part.length).match(/\*\d+/g)[0].slice(1));
            }

            if (prevPart != null) {
                [...prevPart.matchAll(/\d+/g)].forEach(val => {
                    if ((val.index >= startIndex && val.index <= endIndex) || (val.index + val[0].length - 1 >= startIndex && val.index + val[0].length - 1 <= endIndex))
                        nbs.push(+val[0]);
                });
            }

            if (nextPart != null) {
                [...nextPart.matchAll(/\d+/g)].forEach(val => {
                    if ((val.index >= startIndex && val.index <= endIndex) || (val.index + val[0].length - 1 >= startIndex && val.index + val[0].length - 1 <= endIndex))
                        nbs.push(+val[0]);
                });
            }
            ...
            if (nbs.length == 2)
                partNbs.push(nbs.reduce((acc, cur) => acc * cur, 1));
        });
    }
