...
function getCalibration(lines) {    
    let globalCalibration = 0;
    
    lines.forEach((l) => {
        let rep = l.replace(/one/g, 'o1e');
        rep = rep.replace(/two/g, 't2o');
        rep = rep.replace(/three/g, 't3e');
        rep = rep.replace(/four/g, 'f4r');
        rep = rep.replace(/five/g, 'f5e');
        rep = rep.replace(/six/g, 's6x');
        rep = rep.replace(/seven/g, 's7n');
        rep = rep.replace(/eight/g, 'e8t');
        rep = rep.replace(/nine/g, 'n9e');
        
        let digits = rep.replace(/\D/g, '');
        let calibration = parseInt(digits.charAt(0) + digits.charAt(digits.length - 1));
        
        globalCalibration += calibration;
    });
    
    return globalCalibration;
}

console.log(getCalibration(lines));    
console.log(getCalibration(testLines));