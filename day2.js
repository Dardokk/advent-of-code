...
function getResult(games) {
    ...
    for(let i = 0; i < games.length; i++) {
        let game = games[i];
        let redMatches = game.match(/\d+\sred/g);
        let greenMatches = game.match(/\d+\sgreen/g);
        let blueMatches = game.match(/\d+\sblue/g);
        let isPossible = true;

        redMatches.forEach(match => {
            if (match.split(' ')[0] > MAX_RED_CUBES)
                isPossible = false;
        });

        greenMatches.forEach(match => {
            if (match.split(' ')[0] > MAX_GREEN_CUBES)
                isPossible = false;
        });

        blueMatches.forEach(match => {
            if (match.split(' ')[0] > MAX_BLUE_CUBES)
                isPossible = false;
        });
...
function getResult2(games) {
    ...
    for(let i = 0; i < games.length; i++) {
        ...
        let minRedCubes = Math.max(...redMatches.map(m => m.split(' ')[0]));
        let minGreenCubes = Math.max(...greenMatches.map(m => m.split(' ')[0]));
        let minBlueCubes = Math.max(...blueMatches.map(m => m.split(' ')[0]));
        
        powers.push(minRedCubes * minGreenCubes * minBlueCubes);
    }

    return powers.reduce((acc, cur) => acc + cur, 0);
}

console.log(getResult(games));
console.log(getResult2(games));